...
use Base\Message\Model\Conversation;
use Base\Message\Model\Message;
use Base\Message\Model\Recipient;
...
class MessageService
{
     ...
     * @param string $msgId
     *
     * @return \Base\Message\Model\Message
     */
     ...
     * @param $recipientId
     *
     * @return \Base\Message\Model\Recipient
     */
    public function findRecipient($convId, $recipientId)
    {
        return \App::table('base_message_recipient')
            ->select()
    ...
    public function getUnreadConversationCount(PosterInterface $parent = null)
    {
        ...
        $select = \App::table('base_message_recipient')
            ->select()
    ...
    {
        return null != \App::table('base_message_recipient')
            ->select()
            ->where('recipient_id=?', (string)$recipientId)
    ...
    public function loadMessagePaging($query = [], $page = 1, $limit = 12)
    {
        ...
        $messageIdList = \App::table('base_message_recipient')
            ->select()
